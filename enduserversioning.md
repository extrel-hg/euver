...
* 6. Example of the following rules: version C3.0.0=a>C>b can read data from the latest A version that is older than 3.0.0, and the latest B version that is older than 3.0.0 can read data from C3.0.0=a>C>b.
* 7. You SHOULD NOT omit any \<C> tags that fulfill rules 4. or 5.
* 8. You MUST NOT use a \<C> tag for the current version, if you end up with the another tag on both sides of it. Example: B2.0.0=a>B>a. This means the newest version of A can read B2.0.0, and vice versa. You MUST use A instead.
